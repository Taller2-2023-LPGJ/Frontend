...
export const SnapMSG: React.FC<{
  snapMSGInfo: SnapMSGInfo;
  navigation: Navigation;
  scale: number;
  disabled: boolean;
}> = ({ snapMSGInfo, navigation, scale, disabled }) => {
  ...
  const sharePost = () => {
    ...
    Alert.alert(alertTitle, "Do you want to proceed?", [
      { text: "Cancel" },
      {
        text: "Yes",
        onPress: async () => {
  ...
  const replyToPost = () => {
    navigation.navigate("ReplySnapMSG", {
      replyParams: { id: snapMSGInfo.id },
    });
  };
  ...
  const openProfile = async () => {
    ...
    if (snapMSGInfo.author == result) {
      ...
      navigation.navigate("Search", {
        screen: "OtherProfile",
        params: { username: snapMSGInfo.author },
      });
    }
  };
      ...
      >
        {snapMSGInfo.sharedBy != null ? (
          <View style={{ flexDirection: "row", marginBottom: 10 }}>
            ...
            <Text style={{ fontSize: 15 * scale, color: textLight }}>
              SnapShared by{" "}
            </Text>
            <Text
              style={{
        ...
        <View style={styles.row}>
          <View style={{ flexDirection: "row", width: 200 }}>
            ...
            >
              {snapMSGInfo.displayName}
            </Text>
            {snapMSGInfo.verified ? (
              <Icon
                size={15 * scale}
                color={textLight}
            ...
            <Text
              style={{ marginLeft: 5, fontSize: 15 * scale, color: textLight }}
            >
              @{snapMSGInfo.author}
            </Text>
            {snapMSGInfo.editingDate ? (
              <Icon
                size={15 * scale}
                color={textLight}
          ...
          >
            {timeAgo(snapMSGInfo.creationDate)}
          </Text>
        </View>
          ...
          {snapMSGInfo.body.split(" ").map((word, i) => (
            <Text key={i}>
              {word.startsWith("@") || word.startsWith("#") ? (
                <Text style={{ fontSize: 15 * scale, color: accent }}>
                  {word + " "}
                </Text>
              ) : (
                <Text style={{ fontSize: 15 * scale, color: textLight }}>
        ...
        <View style={[styles.row, styles.centeredRow]}>
          <View style={styles.statIcons}>
            <Icon
              size={20 * scale}
              color={isLiked ? accent : textLight}
          ...
          <View style={styles.statIcons}>
            <Icon
              size={20 * scale}
              color={isShared ? "#41628a" : textLight}
              name={"repeat-variant"}
              onPress={sharePost}
            />
            <Text
          ...
          <View style={styles.statIcons}>
            <Icon
              size={20 * scale}
              color={isFavourite ? "#6f7236" : textLight}
...
const FeedTemplate = ({ navigation, feedType, feedParams }: Props) => {
  ...
  const addPost = async () => {
    ...
    try {
      ...
      if (response.data.message != null || response.data.length == 0) {
        ...
        const dataArray = response.data.map(
          (item: {
            author: any;
            body: any;
            creationDate: any;
  ...
  const handleScroll = (event: {
    ...
    if (!endOfFeed) {
      const { layoutMeasurement, contentOffset, contentSize } =
        event.nativeEvent;
      const isAtEnd =
        (layoutMeasurement.height + contentOffset.y) * 1.05 >=
        contentSize.height;
      if (isAtEnd) {
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={
        posts.length > 3
          ...
          : [styles.containerContent, { height: 700 }]
      }
    ...
    >
      ...
      <View style={{ justifyContent: "center", marginVertical: height / 12 }}>
        {endOfFeed ? (
